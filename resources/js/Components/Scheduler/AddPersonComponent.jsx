export const addToPersons = (data, persons) => {
    data.forEach(item => {
      ...
      if (person) {
        ...
        persons.push({ id:item.id, name: item.name,lastname: item.lastname, entryNumber: item.entryNumber, unavailable: [{ start: item.start, end: item.end, project: item.project }] });
      }
